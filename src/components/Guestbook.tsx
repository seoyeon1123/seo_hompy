...
const Guestbook: React.FC<GuestbookProps> = ({ toggleModal }) => {
  ...
  const formatDate = (date: string) => {
    ...
    return d.toLocaleDateString('en-CA'); // 'en-CA' for 'YYYY-MM-DD' format
  };

  return (
    <>
      <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center h-screen items-center">
        <div className="bg-[#16423C] p-8 rounded-lg w-96 shadow-lg transform transition-all duration-300 ease-in-out scale-95 hover:scale-100">
          <h3 className="text-3xl font-bold text-[#E9EFEC] mb-4 text-center">방명록</h3>
